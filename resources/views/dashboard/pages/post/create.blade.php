...
@section('content')
    <div class="card card-lightblue">
        ...
        <!-- /.card-header -->
        <div class="card-body">
            <x-form btn='Create' route='dashboard.posts.store' :files="true">

                <x-form-input col=12 name='title' placeholder='Enter Post Title' type='text'
                    label='Title'></x-form-input>
                <x-text-area col=12 :value="old('body')" name='body' placeholder='Enter Post Body' rows=3 label='Post Body'>
                </x-text-area>
                ...
                <x-select-box col=12 label='Post Category' name='post_category_id'>
                    ...
                    @foreach ($post_categories as $category)
                        <option value="{{ $category->id }}" @selected($category->id == old('post_category_id'))>{{ $category->name }}</option>
                    @endforeach
                </x-select-box>
